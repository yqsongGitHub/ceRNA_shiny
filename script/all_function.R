...
DEGfunc<- function(data,group_list,gpl=NULL,type="array",pvalue=0.05,fc=1.5,ncount=2,inputemiss=F){
  ...
  if(type=="array"){
    ...
    data <- log2(data+0.01)
    data <- data[rowSums(data>1) >ncount,]
...
enrichment_GO_KEGG_Reactome <- function(genesymbol,pvaluecutoff=0.05,qvaluecutoff=1,methods="GO",ShowCategory=5){
  ...
  if(methods=="GO"){
    ...
    p <- dotplot(all,color="pvalue",split="ONTOLOGY",orderBy = "x") + facet_grid(ONTOLOGY~., scale="free")
    print(p)
    return(all)
    ...
    p <- dotplot(KEGG,color="pvalue",orderBy = "x",showCategory=ShowCategory)
    print(p)
    return(KEGG)
    ...
    p <-  dotplot(rea,color="pvalue",orderBy = "x",showCategory=ShowCategory)
    print(p)
    return(rea)
  }
}
...
calculate_miRNA_sponge <- function(miRTargetfile,expfile,data_miRNA,data_all,RNAmap,padj=0.05){
  miRTarget <- read.csv(miRTargetfile)
  DEG.data <- read.csv(expfile, header=FALSE, sep=",", stringsAsFactors = TRUE)
  miRTarget2 <- miRTarget[miRTarget$mir %in% data_miRNA$gene_id & miRTarget$gene %in% data_all$gene_id,]
  ExpDataNames <- c(as.matrix(DEG.data[1,]))
  miRTarget3 <- miRTarget2[miRTarget2[,1] %in% ExpDataNames & miRTarget2[,2] %in% ExpDataNames,]
  sppcceRInt <- spongeMethod(miRTarget3, DEG.data, method = "sppc",padjustvaluecutoff = 1,padjustmethod="fdr",
                             minSharedmiR = 3)
  pcceRInt <- spongeMethod(miRTarget3, DEG.data, method = "pc",padjustvaluecutoff = 1,padjustmethod="fdr",
                           minSharedmiR = 3)
  sppcceRInt1 <- merge(sppcceRInt,RNAmap,by.x="sponge_1",by.y="gene_id")
  sppcceRInt2 <- merge(sppcceRInt1,RNAmap,by.x="sponge_2",by.y="gene_id")
  sppcceRInt3 <- sppcceRInt2[,c(8,10,3:7)]
  colnames(sppcceRInt3)[1:2] <- c("sponge_1","sponge_2")
  pcceRInt1 <- merge(pcceRInt,RNAmap,by.x="sponge_1",by.y="gene_id")
  pcceRInt2 <- merge(pcceRInt1,RNAmap,by.x="sponge_2",by.y="gene_id")
  pcceRInt3 <- pcceRInt2[,c(7,9,3:6)]
  colnames(pcceRInt3)[1:2] <- c("sponge_1","sponge_2")
  sppcceRInt4 <- sppcceRInt3[sppcceRInt3$sponge_1 %in% pcceRInt3$sponge_1 & sppcceRInt3$sponge_2 %in% pcceRInt3$sponge_2,]
  sppcceRInt4 <- sppcceRInt4[sppcceRInt4$`p.adjusted_value of shared miRNAs` <padj,]
  sppcceRInt4 <- sppcceRInt4[order(sppcceRInt4$`#shared miRNAs`,decreasing = T),]
  return(sppcceRInt4) 
}
